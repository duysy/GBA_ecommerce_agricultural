...
export async function setProduct(id, title, price, unit, caterory, datePost, discount, soldAtLocation, hashIpfsDetail) {
    var data = await contract.methods.setProduct(id, title, price, unit, caterory, datePost, discount, soldAtLocation, hashIpfsDetail).encodeABI();
    rawTransaction(data);
}
...
export function setUserInfo(nameUser, addressUser, hashIpfsDetail, phoneNumber) {
    ...
    web3.eth.getTransactionCount(addressFrom, (err, txCount) => {
        ...
        web3.eth.sendSignedTransaction(raw, (err, txHash) => {
            if (err) {
                ...
                // setInterval(() => {
                //     web3.eth.getTransactionReceipt(txHash, (status) => {
                //         console.log(status);
                //     });
                // }, 1000);
            }
