...
contract LinkedList {
    ...
    function sellectAlldata() external view returns (uint256[] memory) {
...
contract LibraryDS {
    ...
    function ranIdByAddress(address x) public view returns (uint256) {
        return (((now * 10**10 + (uint256(x) % 10**10)) % 10**5) + ranUint(5));
    }
...
contract GBA {
    ...
    {
        uint256 addressUser = uint256(address_);
        MapUser[addressUser].listProduct.push(idProduct);
    }
